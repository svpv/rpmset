...
static inline bool unpack20x4c14e4a(const char *s, uint32_t *v, unsigned *e)
{
    ...
    uint32x4_t y = vreinterpretq_u32_u8(vqtbl1q_u8(x6, hi6));
    uint32x4_t x = glue24(glue12(x6));
    *e = (y[0] & 0x60042204) * 0x4011004 >> 28;
    x = vreinterpretq_u32_u8(vqtbl1q_u8(vreinterpretq_u8_u32(x), shuf));
#ifdef SLOW_VSHIFT
    const uint32x4_t vmul = { 256, 2048, 64, 512 };
    x = vmulq_u32(x, vmul);
#else
    const int32x4_t vshift = { 8, 11, 6, 9 };
...
static inline bool unpack21x4c14(const char *s, uint32_t *v, unsigned *e)
{
    ...
    x = vreinterpretq_u32_u8(vqtbl1q_u8(vreinterpretq_u8_u32(x), shuf));
#ifdef SLOW_VSHIFT
    const uint32x4_t vmul = { 128, 1024, 32, 256 };
    x = vmulq_u32(x, vmul);
#else
    const int32x4_t vshift = { 7, 10, 5, 8 };
    x = vshlq_u32(x, vshift);
#endif
    x = vshrq_n_u32(x, 11);
    vst1q_u32(v, x);
...
static inline bool unpack22x4c15e2(const char *s, uint32_t *v, unsigned *e)
{
    ...
    *e = x6[3] >> 4;
    ...
    x = vreinterpretq_u32_u8(vqtbl1q_u8(vreinterpretq_u8_u32(x), shuf));
#ifdef SLOW_VSHIFT
    const uint32x4_t vmul = { 16, 64, 256, 1024 };
    x = vmulq_u32(x, vmul);
#else
    const int32x4_t vshift = { 4, 6, 8, 10 };
    x = vshlq_u32(x, vshift);
#endif
    x = vshrq_n_u32(x, 10);
    vst1q_u32(v, x);
