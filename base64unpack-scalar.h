...
static inline bool unpack12x8c16(const char *s, uint32_t *v, unsigned *e)
{
    ...
    x = base64dec2(s +  2); if (x < 0) return false; else v[1] = x;
    x = base64dec2(s +  4); if (x < 0) return false; else v[2] = x;
    x = base64dec2(s +  6); if (x < 0) return false; else v[3] = x;
    x = base64dec2(s +  8); if (x < 0) return false; else v[4] = x;
    x = base64dec2(s + 10); if (x < 0) return false; else v[5] = x;
    x = base64dec2(s + 12); if (x < 0) return false; else v[6] = x;
